...
#----------------------------------------------------------------------------------------------------------------------#
    # Endre Er{\H o}s
    # SP model based Autogenerated ROS2 {{ model_name }} interfacer node using jinja2
    # V.1.0.2.
#----------------------------------------------------------------------------------------------------------------------#
...
class {{ capitalized_model_name }}Interfacer(Node):

    def __init__(self):
        super().__init__("{{ model_name }}_interfacer")
        ...
        self.{{ model_name }}_sp_to_interfacer_subscriber = self.create_subscription(
            {{ message_type_sp_to_interfacer }}, 
            "/{{ model_name }}_sp_to_interfacer",
            self.{{ model_name }}_sp_to_interfacer_callback,
            10)

        self.{{ model_name }}_driver_to_interfacer_subscriber = self.create_subscription(
            {{ message_type_driver_to_interfacer }},
            "/{{ model_name }}_driver_to_interfacer",
            self.{{ model_name }}_driver_to_interfacer_callback,
            10)

        # Then sleep for a bit so that the node get the updated variables before publishing them
        time.sleep(2)

        self.{{ model_name }}_interfacer_to_sp_publisher_ = self.create_publisher(
            {{ message_type_interfacer_to_sp }},
            "/{{ model_name }}_interfacer_to_sp",
            10)

        self.{{ model_name }}_interfacer_to_driver_publisher_ = self.create_publisher(
            {{ message_type_interfacer_to_driver }},
            "/{{ model_name }}_interfacer_to_driver",
            10)
    ...
    def interfacer_to_sp_publisher_callback(self):
        ...
        self.{{ model_name }}_interfacer_to_sp_publisher_.publish(self.msg_interfacer_to_sp)
    ...
    def interfacer_to_driver_publisher_callback(self):
        ...
        self.{{ model_name }}_interfacer_to_driver_publisher_.publish(self.msg_interfacer_to_driver)

    # SP updating the command variables
    def {{ model_name }}_sp_to_interfacer_callback(self, data):
        {% for item in command_variables %}
        self.{{ item }} = data.{{ item }}
        self.msg_interfacer_to_driver.{{ item }} = self.{{ item }}
        {%- endfor %}
    ...
    def {{ model_name }}_driver_to_interfacer_callback(self, data):
        {% for item in measured_variables %}
        self.{{ item }} = data.{{ item }}
        {%- endfor %}
        {% for item in command_variables %}
...
def main(args=None):
    ...
    {{ model_name }}_interfacer = {{ capitalized_model_name }}Interfacer()

    rclpy.spin({{ model_name }}_interfacer)

    {{ model_name }}_interfacer.destroy_node()
    rclpy.shutdown()
