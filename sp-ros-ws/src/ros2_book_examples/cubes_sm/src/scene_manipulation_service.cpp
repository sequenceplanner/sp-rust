...
SceneManipulationService::SceneManipulationService(
	...
	, scene_manipulation_service_(node_handle->create_service<ManipulateScene>("scene_manipulation_service", std::bind(&SceneManipulationService::onSceneManipulationRequest, this, _1, _2, _3)))
	// , timer_(node_handle->create_wall_timer(std::chrono::milliseconds(100), [this](){ broadcastActivetransforms(); }))
	, transform_broadcaster_(node_handle)
	, tf_buffer_(std::make_shared<rclcpp::Clock>(RCL_SYSTEM_TIME))
  	, transform_listener_(tf_buffer_)
	, static_broadcaster_(node_handle)
