...
class Ros2DornaDriver(Node):
    ...
    def __init__(self):
        ...
        config = '/home/martin/sp/sp-ros-ws/install/dorna/share/dorna/config.yaml'

        self.robot = dorna.Dorna(config)
        self.robot.connect()
        self.robot.home(["j0", "j1", "j2", "j3", "j4"])

        # safe pose: [25, 60, 25, 25, 0]

        #self.robot.play({"command": "move", "prm":{"path": "joint", "movement":0, "joint":[0, 0, 0, 0, 0]}})
        self.robot.play({"command": "move", "prm":{"path": "joint", "movement":0, "joint":[25, 60, 25, 25, 0]}})
...
def main(args=None):
    ...
    ros2_dorna_driver = Ros2DornaDriver()
    rclpy.spin(ros2_dorna_driver)
