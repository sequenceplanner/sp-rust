...
pub struct MessageBuffer<T> where T: std::fmt::Debug {
...
impl<T> MessageBuffer<T> where T: std::fmt::Debug {
    pub fn new (
        buffer_size: usize,
...
impl<T> Future for MessageBuffer<T> where T: std::fmt::Debug  {
    type Item = (); 
    type Error = ();
    ...
    fn poll(&mut self) -> Poll<(), ()> {
        ...
        match getting {
            Ok(Async::Ready(None)) if self.q.is_empty() => {
                Ok(Async::Ready(()))
            }
            Err(e) => {
...
mod messagebuffer_test {
    ...
    fn create() {
        tokio::run( future::lazy(move || {
            ...
            let (buf, to_buf, from_buf) = MessageBuffer::new(2, |last: &mut String, next: &String| {
                false // no merge
            });
            ...
            let getting = from_buf.collect().map(|result| {
                    println!("Yes: {:?}", result);
                    assert_eq!(result, vec!("one", "one", "two", "three", "four"));
                }).map(move |_| ()).map_err(|e| eprintln!("error = {:?}", e));
            tokio::spawn(getting);
    ...
    let (buf, to_buf, from_buf) = MessageBuffer::new(10, |_: &mut State, _: &State| {
            false
        });
