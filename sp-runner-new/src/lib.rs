...
struct RunnerPredicate(StatePath, Predicate);
...
pub struct NewRunner {
    ...
    predicates: Vec<RunnerPredicate>,
}
...
impl NewRunner {
    ...
    fn create_transition_map<'a>(ts: &'a [Transition], specs: &'a [Spec]) -> Vec<Vec<&'a Transition>> {
        let mut temp_xs: Vec<(Vec<&SPPath>, Vec<&Transition>)> = specs.iter().map(|s| {
        ...
        for t in ts.iter() {
            ...
            for (paths, ref mut transitions) in temp_xs.iter_mut() {
                // let paths: &[&SPPath] = paths;
                // let transitions: &mut Vec<&Transition> = transitions;
    ...
    /// 
    fn tick(state: &mut SPState, trans: &[Vec<&Transition>], predicates: &[RunnerPredicate], forbidden_states: &[IfThen]) -> Vec<SPPath> {
        NewRunner::upd_preds(state, predicates);
        trans.iter().flat_map(|ts| {
            let xs = NewRunner::tick_ts(state, ts, forbidden_states);
            NewRunner::upd_preds(state, predicates);
            xs
        }).collect()
    }
    ...
    fn upd_preds(state: &mut SPState, predicates: &[RunnerPredicate]) {
        predicates.iter().for_each(|pr| {
            ...
            if let Err(e) = state.force(&pr.0, value) {
                eprintln!("The predicate {:?} could not be updated in the runner. Got error: {}", pr.0, e);
            }
        })
    }
    
    fn tick_ts(state: &mut SPState, ts: &[&Transition], forbidden_states: &[IfThen]) -> Vec<SPPath> {
        ...
        if enabled {
            ...
            if NewRunner::check_forbidden(state, forbidden_states) {
                ts.iter().flat_map(|t| t.actions.iter()).for_each(|a| {
    ...
    fn check_forbidden(state: &mut SPState, forbidden_states: &[IfThen]) -> bool {
        forbidden_states.iter().any(|f| {
            f._if.eval(state) && f._then.eval(state)
        })
    }
}
...
mod test_new_runner {
    ...
    fn testing_tick() {
        ...
        let ps= vec!(rp);
        let fs = vec!();
        let res = NewRunner::tick(&mut s, &ts, &ps, &fs);
        println!("FIRED: {:?}", res);
    }
    ...
    fn creating_transition_map() {
        ...
        let specs = vec!(
            Spec::new("s1", Transition::new_empty("s1"), vec!(t1.path().clone())),
            Spec::new("s2", Transition::new_empty("s2"), vec!(t2.path().clone(), t3.path().clone())),
            Spec::new("s3", Transition::new_empty("s3"), vec!(t1.path().clone())),
            Spec::new("s4", Transition::new_empty("s4"), vec!()),
        );

        let res = NewRunner::create_transition_map(&ts, &specs);
    ...
    fn testing_large_model() {
        ...
        let ts: Vec<Transition> = (1 .. 100).map(|i| {
            let g = p!{plan == i};
            let a = a!(plan = (i+1));
            Transition::new(&format!("t_{}", i), g, vec!(a), vec!(), false)
        }).collect();

        let upd_ts: Vec<Vec<&Transition>> = ts.iter().map(|t| (
            vec!(t)
        )).collect();
        let x = &upd_ts;
        ...
        let ps= vec!();
        let fs = vec!();
        let res = NewRunner::tick(&mut s, &upd_ts, &ps, &fs);
    }

    fn large_model() {
        let ps: Vec<SPPath> = (1 .. 100).map(|i| {
            SPPath::from_string(&i.to_string())
        }).collect();
