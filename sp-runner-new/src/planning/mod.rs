...
pub fn plan(state: &SPState, goal: &Vec<Predicate>, model: &RunnerModel) -> PlanningResult {
    let result = crate::planning::compute_plan(goal, state, model, 20);
    println!("we have a plan? {} -- got it in {}ms", result.plan_found, result.time_to_solve.as_millis());
    result
}

pub fn convert_planning_result(res: PlanningResult, model: &RunnerModel) -> (Vec<TransitionSpec>, SPState) {
    let ctrl: Vec<SPPath> = model.ab_transitions.ctrl.iter().map(|t: &Transition| t.path().clone()).collect();
    let ctrl_map: std::collections::HashMap<SPPath, Transition> = model.ab_transitions.ctrl.iter().map(|t: &Transition| (t.path().clone(), t.clone())).collect();
